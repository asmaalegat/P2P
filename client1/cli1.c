...
int main(int argc, char *argv[])
{
	...
	printf("1. uploader un fichier sur le serveur \n2.la liste des fichiers sur le serveur \n3. telecharger un fichier\n4. quitter\n\n\n");
	scanf("%d",&option);
	...
	case 1:
	{
		...
		for(i=0;i<no_of_files;i++)
		{
			while(1)
			{
				...
				printf("erreur! Essayez à nouveau\n");
				printf("saisir 0 pour annuler l'operation ou autre pour continuer: \n");
			...
			if(MsgReceiveNum(sock))
				printf("Enregistré à 100 pour 100");
			fclose(fp);
		}
		close(sock);
		break;
	...
	case 3:
	{
		...
		if(peerleft == 1) /* If some peer has left in the middle */
		{
			...
			if(no_of_peers == 1)
			{
				...
				printf("error... Réessayez plus tard.\n");
				break;
			}
